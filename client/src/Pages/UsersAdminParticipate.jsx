...
function UsersAdminParticipate() {
    ...
    async function GetUsers() {
      try {
        ...
        const res = await axios.get(
          `http://localhost:3000/api/admin/getusers/${tripid}`,
          {
  ...
  console.log("Useers Admin", UserParticipants);
  return (
      ...
      <div className="flex flex-col justify-center align-middle mx-10 mt-7">
        {UserParticipants.length > 0 ? (
          UserParticipants.map((UserParticipant, index) => (
            <UserAdminCard key={index} UserParticipants={UserParticipant} />
          ))
        ) : (
...
export default UsersAdminParticipate;
